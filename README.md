...
Current methods each address a piece of this. PPO constrains update size, GRPO removes the value model, DPO moves offline, coverage papers argue for online data, Q* targets token-level credit. But influence allocation, the question of which samples and tokens should receive gradient budget and how much, remains underexplored from first principles.
