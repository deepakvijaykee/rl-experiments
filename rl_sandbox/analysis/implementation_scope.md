...
Every method in this sandbox is a scoped reimplementation of a published method or a deliberately simple reference, kept faithful where the update rule, the credit-assignment scheme, and the normalization choices are concerned, and trimmed where the distributed-system scaffolding around the method does not apply to a single-GPU toy setup. Where the scoping would silently change what a method means, for instance running advantage normalization in a regime where the original paper assumes a critic, the trainer rejects the config rather than running a variant that would be misleading to compare.

Some rows below carry a validity regime rather than a scope note, and the distinction matters. A scope note says a method has been simplified. A validity regime says the derivation behind the method depends on a property of the task, such as binary rewards or grouped rollouts, so running it outside that regime still produces a number, and that number no longer estimates what the method was built to estimate. That failure is invisible in the output, which is why those rows name the regime explicitly.
...
| `TPONoAnchor` | The ablation that drops the old-policy anchor, leaving candidate weights proportional to `exp(skill / eta)`. Isolates how much of TPO's behavior comes from the anchor rather than from the candidate construction. |
...
| Method | Scope here |
| --- | --- |
| `DG` | Delightful policy gradient, faithful to the published gate, meaning `sigmoid(advantage * surprisal / eta)` scaling the advantage. No scoping changes to the update itself. |
| `Kondo` | The compute-efficient variant, screening samples before the learner forward pass by setting the gate threshold to a batch quantile targeting `keep_ratio`, then sampling `Bernoulli(sigmoid((delight - lambda) / eta))`. Faithful to Algorithm 1. The compute saving is real but small here, since the toy learner is cheap, so the arm earns its place through the update semantics rather than the speedup. |
...
| `DGEntropyGuard` | Sampled-action probability guard for entropy-collapse diagnostics, downweighting positive-advantage updates to already-high-probability actions while mostly leaving rare successes alone. It stops short of full covariance control, and that is deliberate: it sits downstream of the standardization on purpose, as the control arm for the entropy result. |
...
| `MaxRL` | Per-group mean-reward normalization instead of std, which is what makes the gradient an unbiased estimate of the maximum-likelihood gradient. Validity regime: binary rewards with grouped rollouts. Outside it the ML connection breaks and `1/mean` stops being a principled weighting. |
