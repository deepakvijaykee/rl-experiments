...
Capacity is the variance knob in replay's variance-vs-bias trade. Age is the bias knob. A comparison that reports buffer capacity alone is tuning along one axis while leaving the other implicit, and conclusions formed that way transfer poorly to setups where the implicit knob sits at a different value. That is the warning the table is meant to deliver.
