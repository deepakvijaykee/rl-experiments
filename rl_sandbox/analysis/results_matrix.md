...
The runs collected here are compact GPU sweeps: three seeds, `batch_size=96`, and the default token-reversal model unless noted. Absolute numbers move with seed and horizon, but the ordering between methods and the qualitative shape of each failure mode have been stable across the variations I checked, which is why the readings below are stated in those terms. The top-level README pulls these six tables into a single narrative, and this document is the source data behind it. Each section gives the run command, the resulting table, and an immediate reading of what the table says.
...
The DGToken row needs one qualification that the table cannot show. Calling its unscored error "damage" presumes the unscored positions deserved no gradient in the first place, and in an autoregressive task that presumption is wrong: the unscored prefix causally conditions the scored suffix, so those positions plausibly deserve some indirect credit that the masked return-to-go scheme never gives them. The honest reading is that DGToken is being scored against a benchmark that is itself an approximation, so its unscored column is evidence that the credit scheme is aggressive, not proof that it is wrong. Separating the two would take a task whose reward genuinely factorizes across positions, which `masked_reversal` does not.

The mechanism behind these readings falls out of the token-level policy gradient itself. Each per-token update is proportional to `grad_theta log pi(a_t | s_t) * A_t`, so at any position where the reward function places no signal the advantage `A_t` is zero, and no upstream sharpening of credit assignment can produce gradient where the multiplicative factor is zero. Token-level methods redistribute existing reward signal across positions, and they cannot manufacture signal where the reward function placed none. That constraint is invisible in the scored column alone, which is why both columns are reported here.
