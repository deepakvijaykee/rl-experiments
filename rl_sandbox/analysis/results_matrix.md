...
The runs collected here are compact GPU sweeps: three seeds, `batch_size=96`, and the default token-reversal model unless noted. Every table reports the mean across those three seeds, with the standard error in parentheses beside it. Absolute numbers move with seed and horizon, so the readings below are stated in terms of what held steady across the variations I checked, meaning the ordering between methods and the qualitative shape of each failure mode. The top-level README pulls these six tables into a single narrative, and this document holds the source data behind it. Each section gives the run command, the resulting table, and an immediate reading of what the table says.
...
Given enough steps, both reviser-based methods reach zero exact-match error before supervised CE does. SelfDistillDG first hits zero around step 467, SCOPELite around step 533, and CE not until step 733. The weak SCOPELite reading I reported at 300 steps was an artifact of the budget. At that horizon the oracle baseline had not finished either, leaving no room for any method to show a speedup, so the number said nothing about SCOPELite. What is worth chasing at scale is SelfDistillDG's 200- to 300-step lead over CE. The oracle reviser carries the toy version, and the next question is whether that lead survives a learned, noisier reviser in its place. Only then does dense correction become a lever for sparse-reward chain tasks, as opposed to a property of having an oracle in the loop.
...
Capacity is the variance knob in replay's variance-versus-bias trade and age is the bias knob. A comparison that reports buffer capacity alone is tuning along one axis while leaving the other implicit, so conclusions formed that way transfer poorly to any setup where the implicit knob happens to sit at a different value. That is what these six rows are for.
...
Both columns carry signal, and reading only the scored column would misrepresent every method in the table. TPOToken drives the scored suffix to zero error while leaving unscored positions near chance, which is the cleanest possible per-position routing result: the credit is allocated where the reward function placed signal, and the method does not pretend to learn at positions the reward function declined to score. DGToken is the cautionary case. It improves the scored suffix relative to sequence-level DG, but it damages the unscored positions, because its return-to-go credit concentrates on positions that received reward and applies a smaller-than-baseline gradient elsewhere. TEMPO is out of regime entirely on this task, and all three seeds stop training early once the grouped rollouts stop producing mixed-reward batches, which is exactly the condition its prefix-tree credit assumes.

The DGToken row needs one qualification the table cannot show. Calling its unscored error "damage" presumes those positions deserved no gradient to begin with, and in an autoregressive task that presumption fails. The unscored prefix causally conditions the scored suffix, so those positions plausibly deserve indirect credit that the masked return-to-go scheme never hands them. DGToken is therefore being scored against a benchmark that is itself an approximation, which makes its unscored column evidence that the credit scheme is aggressive and leaves the question of whether it is wrong open. Settling that would take a task whose reward genuinely factorizes across positions, and `masked_reversal` is not one.

The mechanism behind these readings falls out of the token-level policy gradient itself. Each per-token update is proportional to $\nabla_\theta \log \pi(a_t \mid s_t)\, A_t$, so at any position where the reward function places no signal the advantage $A_t$ is zero, and no upstream sharpening of credit assignment can produce gradient where the multiplicative factor is zero. Token-level methods redistribute existing reward signal across positions, and they cannot manufacture signal where the reward function placed none. That constraint is invisible in the scored column alone, which is why both columns are reported here.
...
| Method | Extra args | Final `test_error` | Final entropy | First entropy $\le 0.1$ |
| --- | --- | --- | --- | --- |
