...
| Influence (delight) | `DG`, `Kondo`, `DGToken` | Delightful Policy Gradient ([arXiv:2603.14608](https://arxiv.org/abs/2603.14608)) and its compute-efficient variant Kondo ([arXiv:2603.20526](https://arxiv.org/abs/2603.20526)). The name is misleading if read as $\text{advantage} \cdot \text{surprisal}$ weighting: that product is the argument to a sigmoid gate, and the gate multiplies the advantage, so surprising successes get amplified and surprising failures get suppressed. Kondo moves the same gate before the learner forward pass and samples it stochastically, so screening saves compute rather than only reweighting. |
...
The matrix and the seven axes in the [repository README](../README.md) are different objects that happen to have the same length, so it is worth saying how they line up. The axes are a taxonomy of design choices. Two of them, privileged rollout acquisition and search coverage under vector rewards, change the batch contract, which is why they live in the companion sandboxes and not here. That leaves the five axes this sandbox can exercise, plus dense correction, which is a follow-up question and not an axis of its own. Six topics across seven rows, because optimization geometry gets two: one row asks what standardization and clipping do to the update, and the other asks when the resulting entropy loss becomes irreversible.
