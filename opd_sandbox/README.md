...
The second is that top-k truncation is a support-overlap assumption that the gradient enforces directly. Reverse KL is an expectation under the student, so every per-action term in the gradient is weighted by `pi_student(a)`. Any teacher mass that lies outside the student's top-k contributes exactly zero to the update. From a random cold start, the student's top-k is largely arbitrary, the teacher's preferred token frequently lands outside it, and the optimizer has no mechanism to pull probability toward what it cannot see. The failure mode is unusual: nothing sharp, just a quiet absence of learning that looks like very slow progress.
